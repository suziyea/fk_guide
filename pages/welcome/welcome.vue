    ...
    <template>
      ...
      <view class="mobilebox">
        ...
        <view class="notice u-flex u-flex-items-center">
          <view
            :class="[!selectRadio ? 'icon-this-option' : 'icon-has-checked']"
          ...
          <text class="read_tip"
            >注册即表明您已经同意<text
              class="blue"
              @click="jumpContent('register')"
              >{{ ` 《注册协议》 ` }}</text
            >和<text class="blue" @click="jumpContent('hide')">{{
              ` 《隐私协议》 `
            }}</text></text
          >
        </view>
      ...
      <view
        class="
          remark
          u-flex u-flex-column u-row-center u-flex-center u-flex-items-center
        "
      >
        <text> 具体放款金额及放款时间视个人情况而定 </text>
        <text> 投资有风险，借贷需谨慎 </text>
        <text> 资金来源于合规持牌机构 </text>
        <navigator
          url="/pages/webview/webview?urlPath=https://beian.miit.gov.cn/#/Integrated/index"
        >
...
<script>
import { register, addDrainage } from "@/config/api/user.js";
export default {
  data() {
    return {
      selectRadio: false,
